...
def test_tracking_pixel_and_click_update_aggregates(client):
    ...
    assert client.get(f"/mailing/t/{token}").status_code==200
...
def test_invalid_pixel_does_not_reveal_validity(client): assert client.get('/mailing/t/invalid').status_code==200
