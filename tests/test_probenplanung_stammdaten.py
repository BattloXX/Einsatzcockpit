...
def test_probeart_crud(client):
    ...
    response = client.post("/probenplanung/verwaltung/probearten", data={
    ...
    response = client.post(f"/probenplanung/verwaltung/probearten/{row_id}", data={
        "_csrf": csrf, "name": "Atemschutz-Vollprobe", "kurz": "ASV", "farbe": "#654321",
        "sortierung": "56", "termin_typ": "uebung", "aktiv": "1"}, follow_redirects=False)
    assert response.status_code == 303
    response = client.post(f"/probenplanung/verwaltung/probearten/{row_id}/loeschen", data={"_csrf": csrf}, follow_redirects=False)
    assert response.status_code == 303


def test_modul_aus_liefert_404(client):
    _user("proben_flag_admin", "org_admin"); _flags(False); _login(client, "proben_flag_admin")
    assert client.get("/probenplanung/verwaltung/probearten").status_code == 404
...
def test_nur_org_admin_verwaltet_probearten(client):
    ...
    assert client.get("/probenplanung/verwaltung/probearten").status_code == 403
