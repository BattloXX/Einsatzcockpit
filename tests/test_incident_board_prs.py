...
def _fahrzeug(db, incident, org):
    vm = VehicleMaster(dept_id=org.id, code="TLFA", name="Tanklöschfahrzeug")
    col = IncidentColumn(incident_id=incident.id, code="active", title="Aktiv", column_kind="vehicles")
    db.add_all([vm, col])
    db.flush()
    vehicle = IncidentVehicle(incident_id=incident.id, column_id=col.id, vehicle_master_id=vm.id)
    db.add(vehicle)
    db.flush()
    return vehicle
