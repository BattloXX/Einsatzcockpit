(() => {
  ...
  const sync = root.querySelector('[data-sync]'); let onlyOpen = false, pending = new Map();
  ...
  function current() { return people().find(p => p.dataset.status === open) || null; }
  function paint() { const next = current(); people().forEach(p => { const active = p === next; p.classList.toggle('appell-person--current', active); p.querySelector('.appell-current').hidden = !active; p.hidden = onlyOpen && p.dataset.status !== open; });
