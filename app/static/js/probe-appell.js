(() => {
  ...
  root.addEventListener('click', event => { const button = event.target.closest('[data-status-action]'); if (button && canEdit) { const person = button.closest('[data-person]'); const status = button.dataset.statusAction; update(person, status); save(person, status); return; }
    if (event.target.closest('[data-filter]')) { onlyOpen = !onlyOpen; paint(); return; } const groupButton = event.target.closest('[data-group-filter]'); if (groupButton) { groupFilter = groupButton.dataset.groupFilter; root.querySelectorAll('[data-group-filter]').forEach(b => b.classList.toggle('is-active', b === groupButton)); paint(); jump(); return; } if (event.target.closest('[data-jump]')) { jump(); return; }
