...
def _preview_recipient_context(user):
    return {
        "vorname": "Max",
        "nachname": "Mustermann",
...
def template_save(
    ...
    item.preheader = preheader or None
    item.body_html = body_html
    item.body_text = body_text or None
    item.description = description or None
    write_audit(
        db,
        "mailing.template.saved",
        org_id=user.org_id,
        user_id=user.id,
...
def template_edit(
    ...
    return templates.TemplateResponse(
        request,
        "mailing/template_form.html",
        ctx(request, user, item=item, test_sent=request.query_params.get("test_sent") == "1"),
    )
...
async def template_test_send(
    ...
    subject, html, plain = render_template(
        item.subject,
        item.body_html,
        item.body_text,
        {
            "vorname": user.display_name.split(" ", 1)[0],
            "nachname": user.display_name.split(" ", 1)[1] if " " in user.display_name else "",
            "email": user.email,
            "empfaenger_name": user.display_name,
            "org_name": getattr(user.org, "name", ""),
        },
    )
    ...
    if config.reply_to_default:
        message["Reply-To"] = config.reply_to_default
    message.set_content(plain or "")
    message.add_alternative(html, subtype="html")
    await send_via_resend(message, api_key, config.from_addr)
    write_audit(
        db,
        "mailing.template.test_sent",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_template",
        entity_id=item.id,
    ...
    db.commit()
    return RedirectResponse(f"/mailing/templates/{template_id}/edit?test_sent=1", 303)
...
def list_new(
    ...
    return templates.TemplateResponse(
        request,
        "mailing/list_form.html",
        ctx(
            request,
            user,
            qualifications=db.query(Qualification).order_by(Qualification.label).all(),
            tags=db.query(MemberTag).order_by(MemberTag.name).all(),
        ),
    )
...
def list_create(
    ...
    if member_since_after:
        filters["member_since_after"] = member_since_after
    if member_since_before:
        filters["member_since_before"] = member_since_before
    obj = service.create_recipient_list(
        db,
        user.org_id,
        name=name,
        kind=kind,
        description=description or None,
        filter_json=json.dumps(filters) if kind == "dynamic" else None,
    )
    write_audit(
        db,
        f"mailing.list.{kind}_created",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_recipient_list",
        entity_id=obj.id,
    )
    db.commit()
...
def campaign_create(
    ...
    track_opens: str = Form(""),
    track_clicks: str = Form(""),
    max_attempts_override: int | None = Form(None),
):
    ids = recipient_list_ids or ([recipient_list_id] if recipient_list_id else [])
    if not ids:
        raise HTTPException(422, "Mindestens eine Empfängerliste ist erforderlich.")
    obj = service.create_campaign(
        ...
        track_opens=track_opens == "1",
        track_clicks=track_clicks == "1",
    ...
    if len(valid) != len(set(ids)):
        raise HTTPException(404)
    for lst in valid:
        db.add(MailingCampaignRecipientList(campaign_id=obj.id, recipient_list_id=lst.id))
    write_audit(
        db,
        "mailing.campaign.created",
        org_id=user.org_id,
        user_id=user.id,
...
def campaign_detail(
    ...
    opened = (
        db.query(MailingQueueItem.opened_at)
        .filter(MailingQueueItem.campaign_id == item.id, MailingQueueItem.opened_at.isnot(None))
        .all()
    )
    clicked = (
        db.query(MailingLinkClick.clicked_at)
        .join(MailingQueueItem, MailingQueueItem.id == MailingLinkClick.queue_item_id)
        .filter(MailingQueueItem.campaign_id == item.id)
        .all()
    )
    max_hour = 0
    ...
    return templates.TemplateResponse(
        ...
        ctx(
            ...
            reaction_opens=[open_map.get(hour, 0) for hour in range(max_hour + 1)],
            reaction_clicks=[click_map.get(hour, 0) for hour in range(max_hour + 1)],
...
async def campaign_send(
    ...
    write_audit(
        db,
        "mailing.campaign.sent_immediate" if immediate else "mailing.campaign.queued",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_campaign",
        entity_id=item.id,
...
def queue_status(
    ...
    return templates.TemplateResponse(
        request,
        "mailing/_queue_status_partial.html",
        ctx(
            request,
            user,
            item=item,
...
def settings_page(
    ...
    return templates.TemplateResponse(
        request,
        "mailing/settings.html",
        ctx(request, user, config=service.get_mailing_config(db, user.org_id), webhook_url=webhook_url),
    )
...
def campaign_schedule(
    ...
    if value is None:
        raise HTTPException(422, "Ungültiger Zeitpunkt")
    item.scheduled_at = value
    item.status = "scheduled"
    write_audit(
        db,
        "mailing.campaign.scheduled",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_campaign",
        entity_id=item.id,
...
def campaign_cancel(
    ...
    item = db.get(MailingCampaign, campaign_id)
    if not item or item.status not in {"draft", "scheduled"}:
        raise HTTPException(409)
    item.status = "cancelled"
    item.cancelled_at = datetime.now(UTC).replace(tzinfo=None)
    write_audit(
        db,
        "mailing.campaign.cancelled",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_campaign",
        entity_id=item.id,
...
async def attachment_upload(
    ...
    write_audit(
        db,
        "mailing.attachment.uploaded",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_campaign_attachment",
        entity_id=obj.id,
    )
    db.commit()
    return RedirectResponse(f"/mailing/campaigns/{campaign_id}", 303)
...
def attachment_delete(
    ...
    campaign = db.get(MailingCampaign, campaign_id)
    obj = db.get(MailingCampaignAttachment, attachment_id)
    if not campaign or campaign.status != "draft" or not obj or obj.campaign_id != campaign.id:
        raise HTTPException(404)
    delete_attachment(db, obj)
    write_audit(
        db,
        "mailing.attachment.deleted",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_campaign_attachment",
...
def tags(
    ...
    return templates.TemplateResponse(
        request, "mailing/tags_list.html", ctx(request, user, items=db.query(MemberTag).order_by(MemberTag.name).all())
    )
...
def tag_create(
    ...
    obj = MemberTag(org_id=user.org_id, name=name.strip(), color=color or None)
    db.add(obj)
    db.commit()
    return RedirectResponse("/mailing/tags", 303)


@router.post("/tags/{tag_id}/delete")
def tag_delete(
    tag_id: int, db=Depends(get_db), user=Depends(require_role("mailing_admin")), _g=Depends(require_mailing_enabled)
):
    obj = db.get(MemberTag, tag_id)
    if not obj:
        raise HTTPException(404)
    for x in db.query(MemberTagAssignment).filter(MemberTagAssignment.tag_id == obj.id).all():
        db.delete(x)
    db.delete(obj)
    db.commit()
    return RedirectResponse("/mailing/tags", 303)
...
def member_tags(
    ...
    if not member:
        raise HTTPException(404)
    for row in db.query(MemberTagAssignment).filter(MemberTagAssignment.member_id == member.id).all():
        db.delete(row)
    valid = db.query(MemberTag).filter(MemberTag.id.in_(tag_ids)).all() if tag_ids else []
    for tag in valid:
        db.add(MemberTagAssignment(member_id=member.id, tag_id=tag.id))
    db.commit()
    return templates.TemplateResponse(
        request,
        "mailing/_member_tags.html",
        ctx(
            request,
            user,
            member=member,
...
def import_incident(
    ...
    for row in resolve_incident_participants(db, user.org_id, incident_id):
        db.add(
            MailingRecipientListEntry(
                org_id=user.org_id, list_id=obj.id, email=row["email"], display_name=row["display_name"]
            )
        )
    write_audit(
        db,
        "mailing.list.incident_imported",
        org_id=user.org_id,
        user_id=user.id,
        entity_type="mailing_recipient_list",
        entity_id=obj.id,
...
def dashboard(
    ...
    return templates.TemplateResponse(
        request, "mailing/dashboard.html", ctx(request, user, **service.build_mailing_dashboard_data(db))
    )


@router.get("/lists/{list_id}/export.csv")
def list_export(
    list_id: int, db=Depends(get_db), user=Depends(require_role("mailing_admin")), _g=Depends(require_mailing_enabled)
):
    import csv
    import io
    ...
    writer.writerow(["E-Mail", "Name"])
    for row in resolve_recipient_list(db, item, incident_id=None):
        writer.writerow([row["email"], row.get("display_name") or ""])
    return Response(
        content=("﻿" + buf.getvalue()).encode("utf-8"),
        media_type="text/csv; charset=utf-8",
        headers={"Content-Disposition": f"attachment; filename=empfaengerliste_{list_id}.csv"},
    )
...
def campaign_export(
    ...
    writer = csv.writer(buf, delimiter=";")
    writer.writerow(
        ["E-Mail", "Name", "Status", "Versendet am", "Geöffnet am", "Öffnungen", "Erster Klick am", "Klicks", "Fehler"]
    )
    for x in (
        db.query(MailingQueueItem).filter(MailingQueueItem.campaign_id == item.id).order_by(MailingQueueItem.id).all()
    ):
        def fmt(v):
            return format_local_datetime(v, user.org) if v else ""
        writer.writerow(
            [
                x.email,
                x.display_name or "",
    ...
    return Response(
        content=("﻿" + buf.getvalue()).encode("utf-8"),
        media_type="text/csv; charset=utf-8",
        headers={"Content-Disposition": f"attachment; filename=mailing_kampagne_{campaign_id}.csv"},
    )
...
def dashboard_report(
    ...
    return templates.TemplateResponse(
        request,
        "mailing/dashboard_report.html",
        ctx(request, user, org=user.org, **service.build_mailing_dashboard_data(db)),
    )
...
def dashboard_report_pdf(
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=mailing_bericht.pdf"},
    )
...
def suppression_list(
    ...
    if q:
        query = query.filter(MailingSuppressionEntry.email.ilike(f"%{q.strip()}%"))
    total = query.count()
    pages = max(1, (total + PAGE_SIZE - 1) // PAGE_SIZE)
    page = min(max(page, 1), pages)
    items = (
        query.order_by(MailingSuppressionEntry.created_at.desc()).offset((page - 1) * PAGE_SIZE).limit(PAGE_SIZE).all()
    )
    return templates.TemplateResponse(
        request,
        "mailing/suppression_list.html",
        ctx(request, user, items=items, q=q, page=page, total_pages=pages, total=total),
    )
...
def suppression_add(
    ...
    hit = db.query(MailingSuppressionEntry).filter(MailingSuppressionEntry.email == clean).first()
    if hit:
        hit.reason = "manual"
        hit.note = note or None
        hit.created_by_user_id = user.id
        hit.created_at = datetime.now(UTC).replace(tzinfo=None)
    else:
        db.add(
            MailingSuppressionEntry(
                org_id=user.org_id, email=clean, reason="manual", note=note or None, created_by_user_id=user.id
            )
        )
    db.commit()
    return RedirectResponse("/mailing/suppression", 303)


@router.post("/suppression/{entry_id}/delete")
def suppression_delete(
    entry_id: int, db=Depends(get_db), user=Depends(require_role("mailing_admin")), _g=Depends(require_mailing_enabled)
):
