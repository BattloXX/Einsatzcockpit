...
logger = logging.getLogger("einsatzleiter.mailing.schedule")
INTERVAL_SECONDS = 60
...
def materialize_due_campaigns(db=None):
    ...
    try:
        ...
        rows = (
            ...
            .filter(MailingCampaign.status == "scheduled", MailingCampaign.scheduled_at <= now)
            .all()
        )
        for row in rows:
            row.status = "draft"
            queue_campaign(db, row)
        db.commit()
        return len(rows)
    finally:
        if owns:
...
async def mailing_schedule_loop():
    while True:
        try:
            ...
            logger.exception("Mailing-Zeitplanung fehlgeschlagen")
