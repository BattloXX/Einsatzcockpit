...
def verify_resend_webhook_signature(
    ...
    *,
    ...
    try:
        ...
        return False
    return any(part.startswith("v1,") and hmac.compare_digest(expected, part[3:]) for part in svix_signature.split())
...
def _suppress(db, org_id: int, email: str, reason: str, event_id: int):
    ...
    hit = (
        ...
        .filter(MailingSuppressionEntry.org_id == org_id, MailingSuppressionEntry.email == email)
        .first()
    )
    ...
    if hit:
        ...
        db.add(
            MailingSuppressionEntry(org_id=org_id, email=email, reason=reason, source_event_id=event_id, created_at=now)
        )


def process_resend_webhook_event(db, org_id: int, event_type: str, payload: dict, svix_id: str) -> MailingWebhookEvent:
    existing = (
        db.query(MailingWebhookEvent)
        .execution_options(include_all_tenants=True)
        .filter(MailingWebhookEvent.svix_id == svix_id)
        .first()
    )
    if existing:
        return existing
    ...
    if message_id:
        item = (
            ...
            .execution_options(include_all_tenants=True)
            .filter(MailingQueueItem.org_id == org_id, MailingQueueItem.resend_message_id == message_id)
            .first()
        )
    now = datetime.now(UTC).replace(tzinfo=None)
    event = MailingWebhookEvent(
        org_id=org_id,
        queue_item_id=item.id if item else None,
        campaign_id=item.campaign_id if item else None,
