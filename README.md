...
| **BMA-Webplattform-Import** | Täglicher Abgleich der Brandmeldeanlagen-Liste der Landeswarnzentrale Vorarlberg (BMA-Webplattform) in die Objektverwaltung: BMA-Nummer, Adresse inkl. Koordinaten, RFL-Status und Kontaktpersonen; unbekannte Anlagen werden als Entwurf-Objekt angelegt, Änderungen an bereits freigegebenen Objekten landen als Vorschlag in einer Review-Queue (Übernahme über eine Arbeitskopie, nie direkt); Session-Cookie-Login (Fernet-verschlüsselt) statt Zugangsdaten, da der Portal-Login eine Anti-Roboter-Verifizierung hat |
...
| `0181_bma_import.py` | **BMA-Webplattform-Import**: `org_bma_import_config`, `bma_import_satz`, `bma_import_lauf`; `objekt_kontakt.extern_quelle`/`extern_id` (unterscheidet importierte von händisch gepflegten Kontakten) |
