...
| **Org-Datensicherung (Self-Service)** | Jede Organisation sichert ihre **eigenen** (tenant-gescopten) Daten selbst: vollständiges Archiv (Einsätze, Mannschaft, Objekte inkl. Medien, Konfig) als **Download** oder **zeitgesteuerter Push** an ein **selbst konfiguriertes Ziel** (SFTP/SCP/rsync/FTP/FTPS/rclone, Zugangsdaten Fernet-verschlüsselt); server-gebundene Secrets/Tokens ausgenommen; **Restore** in eine neue Org mit ID-Remapping (System-Admin) ([Wiki](https://github.com/BattloXX/Einsatzcockpit/wiki/Administration-Org-Datensicherung)) |
...
| **Nachschlagewerke** | Offlinefähiges Einsatz-Nachschlagewerk (`/nachschlagewerke`): Gefahrgut-Suche nach **UN-Nummer oder Stoffname** über den vollständigen ADR-Bestand (~2.350 Stoffe, BAM „Datenbank GEFAHRGUT", dl-de/by-2.0) → ERI-Karte mit Klasse/Kemler/Verpackungsgruppe + Deep-Links (ERICard/BAM); **täglicher automatischer BAM/ADR-Sync** (CSV oder ZIP mit Auto-Entpacken); **Rettungsdatenblätter** für die technische Rettung (on-demand fetch + Cache); Karten-Overlays in der Lageführung: **Evakuierungsradius** (ERG 2020) und **windbezogene Ausbreitung** (Kegel + Gaußsches Fahnenmodell); Offline-Suche per Service-Worker; zweistufiger Feature-Flag |
